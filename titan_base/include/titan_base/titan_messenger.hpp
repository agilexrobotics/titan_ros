...
#ifndef TITAN_MESSENGER_HPP
#define TITAN_MESSENGER_HPP
...
#include <titan_msgs/ActuatorStateArray.h>
#include <titan_msgs/BatteryState.h>
#include <titan_msgs/TitanBrakeCmd.h>
...
class TitanROSMessenger {
  ...
  enum class TitanSubType { kTitan = 0};

 public:
  TitanROSMessenger(ros::NodeHandle* nh);
 ...
 private:
  ...
  // void BrakeCmdCallback(const titan_msgs::TitanBrakeCmd::ConstPtr& msg);

  double CalculateSteeringAngle(geometry_msgs::Twist msg, double& radius);
  void UpdateOdometry(double linear, double angle, double dt);
  // double ConvertInnerAngleToCentral(double angle);
  // double ConvertCentralAngleToInner(double angle);

  ros::NodeHandle* nh_;
  std::shared_ptr<TitanRobot> robot_;
  TitanSubType robot_type_;
  ...
  // ros::Subscriber unlock_brake_sub;
